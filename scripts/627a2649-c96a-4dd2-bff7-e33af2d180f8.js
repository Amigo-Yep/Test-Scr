  ...
  try {
    ...
    while (retries_step5 > 0) {
      try {
        // ⌨️ كتابة نص في حقل
        console.log('\n⌨️ كتابة نص في حقل:');
        console.log('   📍 الحقل: #sb_form_q');
        console.log('   📝 النص: (55Fred55)');
        console.log('   ⏳ جاري ملء الحقل...');
        try {
          await humanType(currentPage, "#sb_form_q", "(55Fred55)");
    ...
    while (retries_step13 > 0) {
      try {
        ...
        console.log('   ⏳ جاري ملء الحقل...');
        try {
          await humanType(currentPage, "#web-ui1", "55Fred55@Gmail.com");
          console.log('   ✅ تم ملء الحقل بنجاح');
        } catch (typeError) {
          console.error('   ❌ خطأ في ملء الحقل:', typeError.message);
          throw typeError;
    ...
    while (retries_step14 > 0) {
      try {
        ...
        console.log('   ⏳ جاري ملء الحقل...');
        try {
          await humanType(currentPage, "#web-ui2", "(55Fred55)");
          console.log('   ✅ تم ملء الحقل بنجاح');
        } catch (typeError) {
          console.error('   ❌ خطأ في ملء الحقل:', typeError.message);
          throw typeError;
    ...
    // Step 16: نقر على عنصر
    let retries_step16 = 2;
    ...
    // Step 21: نقر على عنصر
    let retries_step21 = 2;
    while (retries_step21 > 0) {
      try {
    ...
    // Step 22: انتظار
    let retries_step22 = 3;
    while (retries_step22 > 0) {
      try {
        // ⏱️ انتظار زمني
        console.log('\n⏱️ الانتظار:');
        console.log('   ⏳ المدة: 5000ms (5.0s)');
    ...
    // Step 23: انتظار
    let retries_step23 = 3;
    while (retries_step23 > 0) {
      try {
        // ⏱️ انتظار زمني
        console.log('\n⏱️ الانتظار:');
        console.log('   ⏳ المدة: 5000ms (5.0s)');
    ...
    // Step 24: التقاط صورة
    let retries_step24 = 3;
    while (retries_step24 > 0) {
      try {
        // 📸 التقاط لقطة شاشة
        screenshotCounter++;
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
